...
class VideoRecorder extends CaptureBase {
    ...
    /**
     * Record specific source (screen or window)
     * @param {string} sourceId - Source ID to record
     * @param {Object} options - Recording options
     * @returns {Promise<Object>} Recording start result
     */
    async recordSource(sourceId, options = {}) {
        if (this.isCapturing) {
            throw new Error('Recording already in progress');
        }
        ...
        try {
            ...
            console.error('Failed to record source:', error);
            return {
                success: false,
                error: error.message || 'Failed to record source'
            };
        }
    }

    /**
     * Record with custom constraints
     * @param {Object} constraints - Custom media constraints
     * @param {Object} recordingOptions - Recording options
     * @returns {Promise<Object>} Recording result
     */
    async recordWithConstraints(constraints, recordingOptions = {}) {
        if (this.isCapturing) {
            throw new Error('Recording already in progress');
        }

        try {
            // Get primary source for source ID
            const source = await this.getPrimaryScreenSource();
            
            // Merge custom constraints with source ID
            ...
            // Create stream with custom constraints
            ...
            return {
                success: true,
                message: 'Custom recording started',
            ...
            return {
                success: false,
                error: error.message || 'Failed to start custom recording'
            };
        }
    }
