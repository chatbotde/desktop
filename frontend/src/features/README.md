...
1. **Create a feature definition file:**
   ...
   ```typescript
   // src/features/definitions/voice-input.feature.ts
...
2. **Create an effect file:**
   ...
   ```typescript
   // src/features/effects/voice-input.effect.tsx
...
```typescript
// src/features/definitions/dark-mode.feature.ts
...
**Definition:**
```typescript
// src/features/definitions/always-on-top.feature.ts
...
**Effect:**
```typescript
// src/features/effects/always-on-top.effect.tsx
import { useEffect } from "react"
import { useFeature } from "@/contexts/FeatureContext"
...
```typescript
// src/features/definitions/internal-debug.feature.ts
...
- **Settings → Features Section**: Toggle pills (`src/components/settings/sections/FeaturesSection.tsx`)
- **Feature Context**: Available globally via `useFeature()` hook

The UI component (`src/components/features/feature-active.tsx`) automatically:
...
1. **File Naming**: Feature definition files must be named `*.feature.ts` and placed in `definitions/`
2. **Effect Naming**: Effect files must be named `*.effect.tsx` and placed in `effects/`
...
- ✅ Check file is in `src/features/definitions/`
...
- ✅ Check file is in `src/features/effects/`
...
**Need help?** Check the existing feature definitions in `definitions/` for reference examples.
