...
const DEEPSEEK_PRIMARY_KEY = 'VITE_DEEPSEEK_API_KEY';
const DEEPSEEK_FALLBACK_KEYS = ['DEEPSEEK_API_KEY'];

// Initialize the DeepSeek client
const resolvedKey = resolveEnvValue(DEEPSEEK_PRIMARY_KEY, {
  fallbacks: DEEPSEEK_FALLBACK_KEYS,
  provider: 'DeepSeek',
});

if (!hasValidEnvValue(resolvedKey)) {
  console.warn(
    '[AI Config] DeepSeek API key missing or appears to be a placeholder. Update your .env with a valid value.'
  );
}
...
export class DeepSeekChatService {
  ...
  constructor() {
    if (!hasValidEnvValue(resolvedKey)) {
      console.warn(
        '[AI Config] DeepSeek service is initialized without a valid API key. Attempts to use it will fail until configured.'
      );
    }
  }
  ...
  private async convertMediaToOpenAIFormat(attachment: MediaAttachment): Promise<OpenAI.Chat.ChatCompletionContentPartImage | null> {
    try {
      ...
      console.error('Error converting media for DeepSeek:', error);
      return null;
    }
  }
  ...
  reinitializeWithCurrentModel() {
    // DeepSeek doesn't need reinitialization
  }

  getCurrentModelName(): string {
    return getSelectedModel()?.name || 'deepseek-chat';
  }
}

// Singleton instance
export const deepseekService = new DeepSeekChatService();
...
export function isDeepSeekConfigured(): boolean {
  return hasValidEnvValue(
    resolveEnvValue(DEEPSEEK_PRIMARY_KEY, {
      fallbacks: DEEPSEEK_FALLBACK_KEYS,
      provider: 'DeepSeek',
...
export function getDeepSeekConfigStatus() {
  ...
  return {
    ...
    instructions: isConfigured ? null : [
      ...
      '3. Add: VITE_DEEPSEEK_API_KEY=your_actual_api_key',
