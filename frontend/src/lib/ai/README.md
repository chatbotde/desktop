...
- **Multiple AI Providers**: Google Gemini, OpenAI, Anthropic (planned)
...
- **GPT-4 Turbo**: High-performance text and vision model
...
This document explains how to set up and use the OpenAI integration in your Buddy application.
...
### 🔄 Streaming Responses
The integration supports real-time streaming responses from OpenAI, so you see the AI's response as it's being generated.
...
4. The response will stream in real-time from OpenAI
...
import { runAllOpenAITests } from '@/lib/ai/openai-test';

// Run all tests
runAllOpenAITests().then(() => {
  console.log('OpenAI tests completed');
});
```

Or test individual functions:
```javascript
import { testOpenAIConfiguration, testOpenAIConnection } from '@/lib/ai/openai-test';
...
#### `sendToOpenAI(message: string)`
Sends a message and returns a streaming response.
```javascript
import { sendToOpenAI } from '@/lib/ai/openai';

const stream = await sendToOpenAI('Hello, how are you?');
for await (const chunk of stream) {
  console.log(chunk); // Each chunk of the response
}
```

#### `sendToOpenAIComplete(message: string)`
Sends a message and returns the complete response.
```javascript
import { sendToOpenAIComplete } from '@/lib/ai/openai';

const response = await sendToOpenAIComplete('What is the weather like?');
...
```javascript
import { getOpenAIConfigStatus } from '@/lib/ai/openai';
...
// Get chat history
const history = openaiChat.getChatHistory();

// Clear chat history
openaiChat.clearHistory();

// Add system context
openaiChat.addSystemContext('You are a helpful coding assistant.');
```
...
### Model Configuration
The default model is `gpt-4o`. You can change this by selecting a different model in the UI.
...
#### "Failed to initialize OpenAI chat"
...
### Debug Mode
You can enable detailed logging by opening the browser console and checking for OpenAI-related log messages.
...
OpenAI has rate limits that vary by model and account type. If you encounter rate limit errors:
...
const stream = await sendToOpenAI('Explain React hooks');
let fullResponse = '';
...
When contributing to the OpenAI integration:
