...
export function YoutubeVideoPlayer({ url, className, onRemove, autoPlay = false }: YoutubeVideoPlayerProps) {
    ...
    <div className={cn("relative w-full max-w-[280px] aspect-video rounded-lg overflow-hidden shadow-md border border-white/10 group bg-black shrink-0", className)}>
      ...
      {onRemove && !isPlaying && (
        <button
          ...
          className="absolute top-1 right-1 w-6 h-6 bg-black/60 hover:bg-black/80 text-white rounded-full flex items-center justify-center transition-colors z-10"
          title="Remove"
        >
