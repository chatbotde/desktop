...
export const VoiceSphere: React.FC<VoiceSphereProps> = ({ isActive, volume, level = 0, onClick }) => {
    ...
    // Load Earth Data - using syncExternalStore for lifecycle
    const [earthDataLoaded, setEarthDataLoaded] = useState(false);

    useSyncExternalStore(
        useCallback((_callback) => {
            if (earthDataLoaded) return () => {};
        ...
        useCallback((_callback) => {
            ...
            const animate = (now: number) => {
                ...
                const projectedParticles = allParticles.map(p => {
                    ...
                    return { x: x2d, y: y2d, z: z2, scale, type: p.type, alphaSeed: p.randomOffset }
                })
