...
/**
 * @overlay RecordedImagePlayerOverlay
 * @description Floating viewer for screenshots / uploaded prompt images.
 */
...
import {
  OPEN_RECORDED_IMAGE_PLAYER_EVENT,
  type OpenRecordedImagePlayerDetail,
} from '@/lib/events/recorded-image-player'
...
export function RecordedImagePlayerOverlay() {
    ...
    useCallback(
      (_notify) => {
        const handler = (event: Event) => {
          const custom = event as CustomEvent<OpenRecordedImagePlayerDetail>
          const file = custom.detail?.file
          const src = custom.detail?.src

          clearImage()

          if (file && file.type.startsWith('image/')) {
            const blobUrl = URL.createObjectURL(file)
            ownedSrcRef.current = blobUrl
            setImageSrc(blobUrl)
          } else if (src) {
            setImageSrc(src)
          } else {
        ...
        window.addEventListener(OPEN_RECORDED_IMAGE_PLAYER_EVENT, handler as EventListener)
        return () =>
          window.removeEventListener(OPEN_RECORDED_IMAGE_PLAYER_EVENT, handler as EventListener)
      },
      [clearImage],
    ),
    () => null,
